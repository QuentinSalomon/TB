    ...
    class WebMaterialSelectListRender : WebPropertyRender
    {
        ...
        public override void BuildControl(WebPageBuilder pagebuilder)
        {
            using (StreamReader file = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream(
                "Framework.WebRender.WebMaterialSelectListRender.Files.MaterialListSelect.html")))
            {
            ...
            using (StreamReader file = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream(
                "Framework.WebRender.WebMaterialSelectListRender.Files.MaterialListSelect.css")))
            {
                string tmp = file.ReadToEnd();
            ...
            using (StreamReader file = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream(
                "Framework.WebRender.WebMaterialSelectListRender.Files.MaterialListSelectCallBack.js")))
            {
                string tmp = file.ReadToEnd();
                tmp = tmp.Replace("<%id%>", PropDescription.PropertyInfo.Name);
